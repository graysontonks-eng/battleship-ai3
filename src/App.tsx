...
function App() {
  ...
  const renderShipSVG = (shipName: string, size: number, isHorizontal: boolean) => {
    // All ship artwork is drawn in a horizontal coordinate space (w x h)
    // For vertical ships, the SVG viewBox is portrait and content is rotated via SVG transform
    const w = size * 37;
    const h = 36;

    // For vertical ships: viewBox is portrait (h x w), content rotated inside SVG
    const vbW = isHorizontal ? w : h;
    const vbH = isHorizontal ? h : w;
    const svgW = isHorizontal ? w : h;
    const svgH = isHorizontal ? h : w;
    // Wrap content in a <g> transform for vertical ships to rotate artwork into portrait space
    const wrapVertical = (children: React.ReactNode) => {
      if (isHorizontal) return children;
      // rotate(90) around origin then translate to keep content in view
      return <g transform={`translate(${h}, 0) rotate(90)`}>{children}</g>;
    };

    switch (shipName) {
      case 'Carrier':
        return (
          <svg viewBox={`0 0 ${vbW} ${vbH}`} xmlns="http://www.w3.org/2000/svg" className="ship-overlay-svg" style={{ width: svgW, height: svgH }}>
            <defs>
